...
public class A04Main {
     ...
     * </pre>
     *
     * Note: your detail may not match this as it is file dependent.
     *
     * @param tree The BST to generate the table from.
     */
    private static void characterTable(final BST<Character> tree) {

	final ArrayList<CountedDetail<Character>> array = tree.inOrder();
	int totalCount = 0;

	for (final CountedDetail<Character> detail : array) {
	    totalCount += detail.getCount();
	}
	System.out.println("Char    Count Percent");

	for (final CountedDetail<Character> detail : array) {
	    final int count = detail.getCount();
	    final double percent = (double) detail.getCount() / totalCount * 100;
	    System.out.format("%4s %,8d %7.2f%n", detail.getDetail(), count, percent);
	}
	return;
    }
	...
	for (final char letter : string.toCharArray()) {
	    final CountedDetail<Character> detail = new CountedDetail<Character>(Character.toUpperCase(letter));
	    tree.insert(detail);
	}
	return;
    }
	...
	while (fileScan.hasNextLine()) {
		...
		if (Character.isLetter(c)) {
		    final CountedDetail<Character> key = new CountedDetail<Character>(Character.toUpperCase(c));
		    tree.retrieve(key);
		}
	    }
	}
	...
	System.out.println("Insert detail: " + Arrays.toString(testData));

	for (Integer i : testData) {
	    CountedDetail<Integer> detail = new CountedDetail<>(i);
	    source.insert(detail);
	}
	...
	CountedDetail<Integer> key = new CountedDetail<>(3);
	...
	System.out.println("  Test Operation {expected detail}: actual detail");
